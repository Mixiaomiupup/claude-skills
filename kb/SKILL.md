...
**边界**: X/Twitter 链接 → 拒绝，提示用户使用 `x2md` skill。本 skill 管理除 `具身行业资讯/` 写入外的所有操作（可读取 `具身行业资讯/` 用于检索和综合）。
...
根据用户查询选择合适的搜索方式，**搜索范围覆盖全 vault（包括 具身行业资讯/）**：
...
- **目标节点**: 具身行业资讯（详见 `feishu` skill 的已知知识库结构）
