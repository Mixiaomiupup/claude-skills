...
飞书 API 集成。通过 `lark-cli` + `feishu_publish.py` + `feishu_broadcast.py` 覆盖知识库、文档、消息、多维表格、电子表格。
...
| 任务 | 方法 | 详情 |
|------|------|------|
| **发布新文档到知识库** | `python3 scripts/feishu_publish.py` | 预处理 → 上传 → 导入 → 移入 wiki → 插入图片 |
| **广播推送卡片** | `python3 scripts/feishu_broadcast.py` | 获取可见用户 → 构建卡片 → 私信推送 |
| **更新已有文档** | `lark-cli` Document Block API | 见下方「更新文档」 |
...
```bash
python3 ~/.claude/skills/feishu/scripts/feishu_publish.py \
  --file /path/to/article.md \
  --title "文章标题" \
  --parent-node "UzfowFW4sidwVdkZulecDOQ3nyd"
```

脚本完成：预处理 Markdown（去 frontmatter/wikilink/toc，提取外部图片）→ 上传 → 导入 → 移入 wiki → 下载并插入外部图片（正确比例）→ 返回 doc_token 和 node_token。
...
**Bot 没有删除权限，必须原地修改。** 用 Document Block API：
