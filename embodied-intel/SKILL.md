...
**anyweb 双模式**：`read`/`search`/`screenshot` 走 headless 模式（自动加载保存的 cookie）；Grok 交互（`open`/`click`/`type`/`keys`/`wait`/`state`/`eval`）加 `--cdp` 标志（自动启动独立 Chrome，AI 专用）。交互完成后用 `anyweb close` 关闭 Chrome。
...
```bash
# 1. 打开 Grok 新对话
anyweb --cdp open "https://x.com/i/grok?new=true"
# 2. 等待输入框出现
anyweb --cdp wait selector "textarea[placeholder='Ask anything']" --timeout 15000
# 3. 点击输入框并输入问题（用 type 命令，底层是键盘事件）
anyweb --cdp click "textarea[placeholder='Ask anything']"
anyweb --cdp type "<问题>. Include tweet URLs.\n"

# 4a. 方案 A (推荐): AX Tree 轮询检测 + 读取
# 检测"Copy text"按钮出现 = Grok 回答完成
for i in $(seq 1 60); do
  sleep 3
  anyweb --cdp state --ax -i 2>/dev/null | grep -q '"Copy text"' && break
done
# 读取完整回答（AX tree 包含结构化文本 + URL）
anyweb --cdp state --ax > /tmp/grok_result.txt

# 4b. 方案 B (回退): JS Observer — 当 AX tree 不可用时
anyweb --cdp eval "new Promise(resolve=>{let lastLen=0,maxLen=0,stableCount=0,growthSeen=false;const start=Date.now();const check=()=>{const elapsed=Math.floor((Date.now()-start)/1000);const text=document.body.innerText;const len=text.length;if(len>maxLen)maxLen=len;if(len>lastLen+50){growthSeen=true;stableCount=0}else if(Math.abs(len-lastLen)<10){stableCount++}else{stableCount=0}lastLen=len;if(growthSeen&&stableCount>=3&&elapsed>20){resolve(text.substring(0,8000))}else if(elapsed>120){resolve(text.substring(0,8000))}else{setTimeout(check,3000)}};setTimeout(check,10000)})"
```
...
- **X 登录要求**：anyweb 需要已保存的 X 登录 cookie（`anyweb login x`）。`--cdp` 模式自动启动独立 Chrome（AI 专用）。交互完成后用 `anyweb close` 关闭 Chrome。
